...
export const updateMember = async (id, updatedData) => {
  try {
    ...
    console.log(participation,totalPa)
    const { data, error } = await supabase
      .from('members_orgs')
      .update(cleanedData)
      .eq('id', id);
...
export const handleParticipation =async(userId, event, musicianRole,user) =>{
  try {
        ...
        .insert([
          {
            ...
            content: `${user.name || 'A user'} has joined the event '${event.event_title}'.`,
            sent_at: new Date(),
          },
        ]);
